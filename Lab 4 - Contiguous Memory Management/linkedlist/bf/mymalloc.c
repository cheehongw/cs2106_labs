...
void *mymalloc(size_t size) {
    ...
    while(1) {
        ...
        if ((curr->pdata->status == 0 && curr->pdata->val >= size) && 
                (curr->pdata->val < currBest || currBest < 0)) {

            currBestFit = curr;
            currBest = curr->pdata->val;
        }
    }
